...
CTRL_DIR="$SCRIPT_DIR/../../src/controllers"

# Validar que exista el directorio
if [ ! -d "$CTRL_DIR" ]; then
  # Fallback a directorio local si se ejecuta directamente desde la raíz
  CTRL_DIR="$(pwd)/src/controllers"
fi

if [ ! -d "$CTRL_DIR" ]; then
  echo "❌ Error: No se encontró el directorio de controladores en $CTRL_DIR"
  exit 1
fi

echo "🔧 Parcheando controllers para PostgreSQL en: $CTRL_DIR"
...
for f in "$CTRL_DIR"/*.js; do
  ...
  # 7. Eliminar .populate() y .lean() y .exec()
  sed -i 's/\.populate([^)]*)//' "$f"
  sed -i 's/\.lean()//' "$f"
  sed -i 's/\.exec()//' "$f"

  # 8. Reemplazar findByIdAndUpdate/findByIdAndDelete por equivalentes PG shims
  ...
  # 10. passwordHash y mustChangePassword
